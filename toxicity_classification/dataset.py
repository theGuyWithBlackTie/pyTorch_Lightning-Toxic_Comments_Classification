...
class ToxicCommentsDataset(Dataset):
    def __init__(self, data_path: str, labels: list, tokenizer_name: str = 'bert-base-uncased', max_length: int = 128):
    ...
    def __getitem__(self, index: int):
        ...
        return {
            ...
            'token_type_ids': torch.tensor(inputs['token_type_ids'], dtype=torch.float),
...
class ToxicCommentsDataModule(pl.LightningDataModule):
    def __init__(self, train_data_path: str, val_data_path: str, test_data_path: str, labels: list,
                 tokenizer_name: str = 'bert-base-uncased', max_length: int = 128, batch_size: int = 16):
        super().__init__()
    ...
    def train_dataloader(self):
        return DataLoader(dataset=self.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=5)

    def val_dataloader(self):
        return DataLoader(dataset=self.val_dataset, batch_size=self.batch_size, shuffle=False, num_workers=1)

    def test_dataloader(self):
        return DataLoader(dataset=self.test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=1)